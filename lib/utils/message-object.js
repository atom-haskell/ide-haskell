...
export default class MessageObject {
  ...
  static validate (message) {
    if (message.text && message.html) throw new Error('Can\'t have both text and html set')
    if (message.highlighter && !message.text) throw new Error('Must pass text when highlighter is set')
    if (!message.text && !message.html) throw new Error('Neither text nor html is set')
  }
