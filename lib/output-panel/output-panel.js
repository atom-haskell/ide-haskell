...
export class OutputPanel {
  constructor(state, results) {
    ...
    this.disposables.add(atom.tooltips.add(this.refs.status, {
      ...
      title: () => {
        res = []
        for ([plugin, {status, detail}] of this.statusMap.entries())
          res.push(```
  ...
  addPanelControl(element, opts) {
    if(typeof element == 'string') {
      let {events, classes, style, attrs, before, id} = opts
      let props = {}
      if(classes)
  ...
  backendStatus(pluginName, {status, progress, detail}) {
    ...
    [consensus] = Array.from(this.statusMap.values()).sort((a, b) => prio[b.status] - prio[a.status])
