...
export class OutputPanel {
    ...
    render() {
        ...
        return (etch.dom("ide-haskell-panel", { style: { width: this.state.width, height: this.state.height }, dataset: { pos: this.pos }, class: this.hiddenOutput ? 'hidden-output' : '' },
            etch.dom("resize-handle", { on: { mousedown: this.resizeStart } }),
            etch.dom("ide-haskell-panel-heading", { ref: 'heading' },
                etch.dom("ide-haskell-status-icon", { ref: 'status', id: 'status', dataset: { status: 'ready' } }),
                etch.dom(OutputPanelButtons, { ref: 'buttons', id: 'buttons' }),
                etch.dom(OutputPanelCheckbox, { ref: 'checkboxUriFilter', id: 'checkboxUriFilter', enabled: this.state.fileFilter }),
                Array.from(this.elements.values()),
                etch.dom(ProgressBar, { ref: 'progressBar', id: 'progressBar', orientation: orientMap[this.pos] })),
            etch.dom(OutputPanelItems, { model: this.results, ref: 'items', id: 'items' })));
    }
    resizeStart(e) {
        let varsMap = {
