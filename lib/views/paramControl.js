...
import selectListView from '../output-panel/views/param-select-view'
...
export class ParamControl {
  constructor ({pluginName, name, spec, value}, children) {
    ...
    this.spec = spec
    this.name = name
    if (value === undefined) { this.value = spec.default } else { this.value = value }
    ...
    if (!this.spec.displayName) { this.spec.displayName = this.name.charAt(0).toUpperCase() + this.name.slice(1) }
    etch.initialize(this)
    this.disposables.add(atom.tooltips.add(this.element,
      {title: () => {
        if (this.hiddenValue) { return `${this.spec.displayName}: ${this.spec.displayTemplate(this.value)}` } else { return this.spec.displayName }
      }}))
  }
