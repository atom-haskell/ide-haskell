...
export class UPI {
  ...
  */
  ...
  public withEventRange<T>({editor, detail, eventType, pos, controller}: TEventRangeParamsInternal, callback: TEventRangeCallback<T>) {
  ...
  public getEventRange({editor, detail, eventType, pos, controller}: TEventRangeParamsInternal) : {pos: Point, crange: Range} | undefined {
    if (pos) pos = Point.fromObject(pos)
    if (!eventType) eventType = getEventType(detail)
    if (!controller) controller = this.pluginManager.controller(editor)
    if (!controller) return
